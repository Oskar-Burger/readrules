...
create.match.list<-function(An, Bn, key){
  ...
  if (is.data.frame(An)) An <- colnames(An)
  if (is.data.frame(Bn)) Bn <- colnames(Bn)
  key <- data.frame(key)
...
Adf <- data.frame(A1=runif(10),A2=letters[1:10],A3=1:10,A4=LETTERS[1:10],A5=1:2,A6=1:5,C8=3,R=0,Q=0)
Bdf <- data.frame(B1=runif(10),B2=letters[11:20],B3=11:20,B4=LETTERS[8:17],B5=1:2,B6=1:5,C7=2,Z=0,QQ=0)
...
KEY <- data.frame(A=      c('A1','A3','A4','A6','C8'),
                  B=      c('B1','B3','B4','B6','C7'),
...
ad_match.list<-create.match.list(adint, partsurv, key_adintpartsurv)

chi_match.list<-create.match.list(chiint, partsurv, key_chiintpartsurv)

complex_rbind(chiint, partsurv, match.list, gr.nam=1:2, gr.lab='Task')

complex_rbind(adint, partsurv, match.list, gr.nam=1:2, gr.lab='Task')
